...
router.get("/", async (req, res) => {
  const { rows } = await db.query(
    `SELECT u.id, u.nombre, u.apellido, u.usuario, u.rol, u.activo, u.sucursal_id, u.creado_en, u.telefono, s.nombre AS sucursal_nombre
...
/** Crear un bombero o admin nuevo. */
router.post("/", async (req, res) => {
  const { nombre, apellido, usuario, password, rol, sucursal_id, telefono } = req.body || {};
  ...
  try {
    ...
    const { rows } = await db.query(
      `INSERT INTO usuarios (nombre, apellido, usuario, password_hash, rol, sucursal_id, telefono)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, nombre, apellido, usuario, rol, sucursal_id, activo, telefono`,
      [nombreNormalizado, apellidoNormalizado, usuario, hash, rol, sucursal_id || null, telefono || null]
    );
...
/** Editar (activar/desactivar, cambiar clave, cambiar sucursal). */
router.put("/:id", async (req, res) => {
  const { nombre, apellido, sucursal_id, activo, password, telefono } = req.body || {};
  let hash = null;
  ...
  const { rows } = await db.query(
    `UPDATE usuarios SET
     ...
     WHERE id = $7
     RETURNING id, nombre, apellido, usuario, rol, sucursal_id, activo, telefono`,
    [nombreNormalizado, apellidoNormalizado || null, sucursal_id, activo, hash, telefono || null, req.params.id]
  );
